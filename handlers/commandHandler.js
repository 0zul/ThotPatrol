...
module.exports = async message => {
  try {
    ...
    // Checks if ThotPatrol has the required permission
    if (cmd.help.botPermission) {
      if (!message.channel.permissionsFor(message.guild.me) || !message.channel.permissionsFor(message.guild.me).has(cmd.help.botPermission)) {
        /**
        * ThotPatrol has missing permissions.
        * @fires ThotPatrolMissingPermissions
        */
        return message.client.emit('ThotPatrolMissingPermissions', cmd.help.botPermission, message);
      }
    }
