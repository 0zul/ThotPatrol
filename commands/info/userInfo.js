...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (message.mentions.users.size) {
    ...
    member = await ThotPatrol.utils.fetchMember(message.guild, args.id);
    if (member) {
      user = member.user;
    }
  }
  ...
  if (!member) {
    member = await ThotPatrol.utils.fetchMember(message.guild, user.id);
  }
  ...
  if (user.presence.game) {
    activity = `${ThotPatrol.Constants.ActivityTypes[user.presence.game.type]} ${user.presence.game.name}`;
  }
  else {
    activity = 'None';
  }
  ...
  let mutualGuilds = await ThotPatrol.methods.getMutualGuilds(user);
