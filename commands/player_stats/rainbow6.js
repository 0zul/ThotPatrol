...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (!/^(uplay|ps4|xone)$/.test(args[0] = args[0].toLowerCase())) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'invalidPlatform', '`Uplay`, `PS4` and `XOne`'), message.channel);
  }
  if (!/^[a-zA-Z][\w-. ]{2,14}$/.test(args[1] = args.slice(1).join(' '))) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'invalidInput', 'username'), message.channel);
  }
