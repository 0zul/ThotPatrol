  ...
  try {
    ...
    if (
      ...
      return ThotPatrol.emit(
        'error',
        '',
        ThotPatrol.i18n.error(message.guild.language, 'needleTooShort', '4'),
        message.channel
      );
    }
    ...
    let options = {
      ...
      headers: {
        Authorization: `Bearer ${ThotPatrol.credentials.clashOfClansAPIKey}`,
        'User-Agent': `ThotPatrol/${ThotPatrol.package.version} (${
          ThotPatrol.user.tag
        }; ${ThotPatrol.user.id}) https://bastionbot.org`
      },
      json: true
    };
    ...
    if (result.error) {
      return ThotPatrol.emit('error', 'Error', result.error, message.channel);
    }

    // Create embed
    let embed = new Discord.RichEmbed().
      setColor(ThotPatrol.colors.BLUE).
      setTimestamp();
    ...
    if (e.name === 'StatusCodeError') {
      if (e.statusCode === 404) {
        return ThotPatrol.emit('error', 'No search results', '', message.channel);
      }
        ...
        return ThotPatrol.emit('error', 'Unknown error', '', message.channel);
      }
