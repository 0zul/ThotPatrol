...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (!modeRegExp.test(args.mode)) {
    return ThotPatrol.emit('error', '', `You have entered an invalid game mode. Valid game modes are: ${gameModes.join(', ').toUpperCase()}`, message.channel);
  }

  let stats = await ThotPatrol.methods.makeBWAPIRequest(`/gamestats/pubg/steam/${args.player}/lifetime`);
