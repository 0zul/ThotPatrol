...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  let options = {
    headers: {
      'X-API-Key': `${ThotPatrol.credentials.bungieAPIKey}`
    },
  ...
  if (player.ErrorCode !== 1) {
    return ThotPatrol.emit('error', player.ErrorStatus, player.Message, message.channel);
  }
  if (!player.Response.length) {
    return ThotPatrol.emit('error', 'Not Found', 'No players were found for the given name.', message.channel);
  }
  ...
  if (stats.ErrorCode !== 1) {
    return ThotPatrol.emit('error', stats.ErrorStatus, stats.Message, message.channel);
  }
