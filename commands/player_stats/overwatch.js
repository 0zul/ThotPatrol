...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (args.region && !/^(us|eu|kr|cn)$/.test(args.region.toLowerCase())) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'invalidRegion', '`US`, `EU`, `KR` and `CN`'), message.channel);
  }
  if (args.platform && !/^(pc|psn|xbl)$/.test(args.platform.toLowerCase())) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'invalidPlatform', '`PC`, `PSN`, `XBL`'), message.channel);
  }


  let stats = await ThotPatrol.methods.makeBWAPIRequest(`/gamestats/overwatch/${args.platform}/${args.region}/${args.battletag.replace('#', '-')}`);

  if (stats.error) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'notFound', 'player'), message.channel);
  }
