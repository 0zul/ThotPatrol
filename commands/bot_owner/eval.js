...
exports.exec = async (ThotPatrol, message, args) => {
  try {
    ...
    let evaled;
    if (args.broadcast && ThotPatrol.shard) {
      evaled = await ThotPatrol.shard.broadcastEval(args.code);
    }
    else {
...
exports.help = {
  ...
  example: [ 'eval message.guild.members.size', 'eval ThotPatrol.users.size --delete' ]
};
 ...
 */
function clean(ThotPatrol, text) {
  text = text.toString();
  if (text.includes(ThotPatrol.token)) {
    text = text.replace(ThotPatrol.token, 'Not for your evil :eyes:!');
  }
