...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (args.cronExp.length !== cronExpLength) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'invalidInput', '`cron` expression'), message.channel);
  }
  ...
  if (!ThotPatrol.commands.has(args.command) && !ThotPatrol.aliases.has(args.command)) {
  ...
  await ThotPatrol.database.models.scheduledCommand.create({
