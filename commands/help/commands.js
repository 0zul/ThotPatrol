...
exports.exec = async (ThotPatrol, message, args) => {
  let categories = ThotPatrol.commands.map(c => c.config.module.toLowerCase()).unique();
  ...
  if (!args.category) {
    return await message.channel.send({
      embed: {
        ...
        footer: {
          text: `Did you know? There are ${ThotPatrol.commands.size} commands in this version of ThotPatrol!`
        }
      }
    });
  }
  ...
  let commands = ThotPatrol.commands.filter(c => c.config.module === args.category);
...
/*        {
      ...
      footer: {
        text: `Did you know? There are ${ThotPatrol.commands.size} commands in this version of Thot Patrol!`
      }
    }
  });
};
...
exports.help = {
  name: 'commands',
  description: 'Shows the list of command categories. And if a category is specified, ThotPatrol will show a list of commands in that category.',
  botPermission: '',
  userTextPermission: '',
  userVoicePermission: '',
