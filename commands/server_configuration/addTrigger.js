...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (!ThotPatrol.credentials.ownerId.includes(message.author.id) && triggerModels && triggerModels.length >= 10) {
    return ThotPatrol.emit('error', 'forbidden', 'You can\'t set more than 10 triggers per server, for now. This limit will be increased in the future.', message.channel);
  }
  ...
  await ThotPatrol.database.models.trigger.create({
    guildID: message.guild.id,
...
exports.help = {
  name: 'addTrigger',
  description: 'Adds a message trigger and a response. When a message is sent that contains that triggering word/phrase, ThotPatrol replies with the response you have set.',
  botPermission: '',
