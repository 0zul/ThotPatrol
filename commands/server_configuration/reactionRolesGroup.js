...
exports.exec = async (ThotPatrol, message, args) => {
  let reactionRolesGroupModels = await ThotPatrol.database.models.reactionRolesGroup.findAll({
    fields: [ 'messageID' ],
    where: {
  ...
  if (!args.roles) {
    ...
    if (args.remove) {
      if (reactionRolesGroups.includes(args.remove)) {
        await ThotPatrol.database.models.reactionRolesGroup.destroy({
          where: {
  ...
  if (reactionRolesGroupModels && reactionRolesGroupModels.length === 2) {
    return ThotPatrol.emit('error', '', 'You can\'t have more than 2 Reaction Roles Group, for now. Delete any previous Group of Reaction Roles to add new ones.', message.channel);
  }


  let roleModels = await ThotPatrol.database.models.role.findAll({
  ...
  if (unassignedRoleEmojis) {
    return ThotPatrol.emit('error', '', 'Some roles doesn\'t have any assigned emojis. Please assign emojis to roles before you use them for reaction roles.', message.channel);
  }
  ...
  await ThotPatrol.database.models.reactionRolesGroup.upsert({
