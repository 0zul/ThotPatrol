...
exports.exec = async (ThotPatrol, message, args) => {
  if (!message.guild.available) return ThotPatrol.log.info(`${message.guild.name} Guild is not available. It generally indicates a server outage.`);

  let muteRole = message.guild.roles.find(role => role.name === 'ThotPatrol:mute'), mutedMembers = [];
  if (muteRole) {
