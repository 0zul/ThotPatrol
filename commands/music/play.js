...
exports.exec = async (ThotPatrol, message, args) => {
  try {
    ...
    if (voiceConnection) {
      ...
      vcStats = ThotPatrol.i18n.error(message.guild.language, 'userNoSameVC', message.author.tag);
    }
    else {
      if (message.guild.music.textChannelID && message.guild.music.voiceChannelID) {
        ...
        vcStats = ThotPatrol.i18n.error(message.guild.language, 'userNoMusicChannel', message.author.tag, voiceChannel.name);
      }
      else if (ThotPatrol.credentials.ownerId.includes(message.author.id) || message.member.roles.has(message.guild.music.masterRoleID)) {
    ...
    if (textChannel.id !== message.channel.id) {
      return ThotPatrol.log.info(`Music commands will only work in the ${textChannel.name} text channel for this session.`);
    }
    if (!voiceChannel.members.get(message.author.id)) {
      return ThotPatrol.emit('error', '', vcStats, message.channel);
    }
    ...
    if (args.playlist) {
      // ThotPatrol Playlist
      let playlistModel = await ThotPatrol.database.models.playlist.findOne({
        attributes: [ 'songs' ],
        where: {
      ...
      if (!playlistModel || !playlistModel.dataValues.songs.length) {
        return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'playlistNotFound'), message.channel);
      }
      ...
      let youtubeDLOptions = [
        ...
        `--user-agent=ThotPatrolDiscordBot/v${ThotPatrol.package.version} (https://bastionbot.org)`,
      ...
      if (!songInfo || (!playlist && (!songInfo.format_id || songInfo.format_id.startsWith('0')))) {
        ThotPatrol.log.error(songInfo);
        return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'notFound', 'result'), message.channel);
      }
...
exports.help = {
  name: 'play',
  description: 'Plays a song or adds the song to the current music queue if ThotPatrol is already playing in your Discord server, specified either by the name, or the link, or from your favorite songs list, or from a YouTube playlist link.',
  botPermission: '',
  userTextPermission: '',
  userVoicePermission: '',
 ...
 * @param {VoiceConnection} connection the VoiceConnection of ThotPatrol in this guild
 * @returns {void}
 */
async function startStreamDispatcher(guild, connection) {
  if (!guild.music.songs[0] && guild.music.autoPlay && connection.channel.members.size > 1) {
    ...
    let youtubeDLOptions = [
      ...
      '--no-warnings',
      '--format=bestaudio[protocol^=http]',
      '--user-agent=ThotPatrolDiscordBot (https://bastionbot.org)',
      '--referer=https://bastionbot.org',
      '--youtube-skip-dash-manifest'
    ];
