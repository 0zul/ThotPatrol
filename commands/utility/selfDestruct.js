...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (!args.timeout.inRange(minTimeout, maxTimeout)) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'selfDestructTimeout', minTimeout, maxTimeout), message.channel);
  }
  ...
  let secretMessage = await message.channel.send({
    embed: {
      color: ThotPatrol.colors.DEFAULT,
