...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (!/^(http[s]?:\/\/)?(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_+.~#?&/=]*)$/.test(args.url)) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'invalidInput', 'URL'), message.channel);
  }

  let webshot = await ThotPatrol.methods.makeBWAPIRequest('/url/capture', {
    encoding: null,
    qs: {
      url: args.url
    }
...
exports.help = {
  ...
  userVoicePermission: '',
  usage: 'capture <url>',
  example: [ 'capture https://shirobot.gq' ]
};
