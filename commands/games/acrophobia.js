...
exports.exec = async (ThotPatrol, message) => {
  if (activeChannels.hasOwnProperty(message.channel.id))  {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'isGameInUse', 'acrophobia'), message.channel);
  }
  ...
  for (let i = 0; i < acroLen; i++) {
    acronym.push(ThotPatrol.methods.getRandomChar(true).toUpperCase());
  }
