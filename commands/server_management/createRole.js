...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (!ThotPatrol.resolver.resolveColor(args.color)) {
    args.color = 0;
  }
  ...
  if (!args.name.length.inRange(0, maxLength)) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'roleNameLength', maxLength), message.channel);
  }
  ...
  await message.channel.send({
    embed: {
      ...
      description: ThotPatrol.i18n.info(message.guild.language, 'createRole', message.author.tag, role.name),
      footer: {
        text: `ID: ${role.id}`
      }
    }
