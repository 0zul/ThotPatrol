...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (args.item) {
    ...
    if (!args.timeout.inRange(1, 24)) {
      return ThotPatrol.emit('error', '', 'Giveaway can only run for at least an hour and at most 24 hours.', message.channel);
    }
    ...
    let giveaway = ThotPatrol.setTimeout(async () => {
      ...
      if (participants.length) {
        winners = ThotPatrol.methods.getRandomElements(participants, args.winners, true);
      }
    ...
    if (message.guild.giveaways.has(args.reroll)) {
      return ThotPatrol.emit('error', ThotPatrol.i18n.error(message.guild.language, 'notFound'), 'That giveaway is currently running in this server. You can only reroll concluded or abruptly stopped giveaways.', message.channel);
    }
    ...
    if (giveawayMessage.author.id !== ThotPatrol.user.id || giveawayMessage.embeds.length !== 1 || !giveawayMessage.embeds[0].author.name.startsWith('GIVEAWAY')) return;
    ...
    if (message.guild.giveaways.has(args.end)) {
      ...
      ThotPatrol.clearTimeout(message.guild.giveaways.get(args.end));
      ...
      return ThotPatrol.emit('error', '', 'There\'s no giveaway running in this server for the specified ID.', message.channel);
    }
  }
  else {
...
exports.help = {
  ...
  example: [ 'giveaway Awesome Goodies! -t 2', 'giveaway ThotPatrol T-Shirt --winners 5', 'giveaway --reroll 133174241744538617', 'giveaway --end 153174267544338344' ]
};
