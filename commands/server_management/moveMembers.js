...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  if (!sourceVoiceChannel || (args.to && !destinationVoiceChannel)) {
    return ThotPatrol.emit('error', '', 'Invalid voice channel(s). Please recheck the IDs of the Voice Channels.', message.channel);
  }
  ...
  for (let member of sourceVoiceChannelMembers) {
    await member.setVoiceChannel(destinationVoiceChannel).catch((e) => ThotPatrol.log.error(e));
  }
...
exports.help = {
  name: 'moveMembers',
  description: 'Moves all the members in a voice channel to another specified voice channel. If the destination voice channel is not specified, ThotPatrol will instead kick all the members from the source voice channel.',
