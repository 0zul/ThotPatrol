...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  const requestOptions = {
    ...
    qs: {
      key: ThotPatrol.credentials.googleAPIkey,
  ...
  if (books.totalItems < 1) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'notFound', 'book'), message.channel);
  }

  const { items: [ book ] } = books;

  await message.channel.send({
    embed: createEmbed(book.volumeInfo, ThotPatrol)
  }).catch(e => {
 ...
 * @param {object} ThotPatrol ThotPatrol client object
 * @returns {object} The embed object of the book
 */
const createEmbed = (volumeInfo, ThotPatrol) => {
  const embed = {
