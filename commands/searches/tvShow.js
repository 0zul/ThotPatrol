...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  let tvShow = await ThotPatrol.methods.makeBWAPIRequest(`/tvshows/search/${args.tvshow}`);

  tvShow = tvShow.results[0];

  if (!tvShow) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'notFound', 'TV show'), message.channel);
  }
