...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  let games = await ThotPatrol.methods.makeBWAPIRequest(`/games/search/${args.name}`);
  let game = games[0];

  if (!game) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'notFound', 'game'), message.channel);
  }
