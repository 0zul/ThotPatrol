  ...
  await yt.getInfo(args, [ '-q', '--skip-download', '--no-warnings', '--format=bestaudio[protocol^=http]' ], async (err, info) => {
    if (err || info.format_id === undefined || info.format_id.startsWith('0')) {
      ...
      if (err && err.stack.includes('No video results')) {
        errorMessage = ThotPatrol.i18n.error(message.guild.language, 'notFound', 'video');
      }
      else {
        errorMessage = ThotPatrol.i18n.error(message.guild.language, 'connection');
      }
      return ThotPatrol.emit('error', '', errorMessage, message.channel);
    }
