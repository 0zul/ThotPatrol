...
exports.exec = async (ThotPatrol, message, args) => {
  ...
  let movie = await ThotPatrol.methods.makeBWAPIRequest(`/movies/search/${args.movie}`);

  movie = movie.results[0];

  if (!movie) {
    return ThotPatrol.emit('error', '', ThotPatrol.i18n.error(message.guild.language, 'notFound', 'movie'), message.channel);
  }
